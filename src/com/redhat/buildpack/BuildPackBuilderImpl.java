...
import com.redhat.buildpack.docker.*;
...
public class BuildPackBuilderImpl implements BuildPackBuilder{
    ...
    public BuildPackBuilder withContent(String filepath, long length, InputStream filecontent) throws Exception{
    ...
    public BuildPackBuilder withContent(ContainerEntry... entries) throws Exception{
		for(ContainerEntry ce : entries){ this.applicationContent.addLast(ce); }
		return this;
	}
    ...
    public int build(LogReader logger) throws Exception{
		...
		String id = ContainerUtils.createContainer(dc, buildImage, Arrays.asList(args),
				...
				new VolumeBind(dockerSocket,"/var/run/docker.sock"),
	...
	//daft little adapter class to read log output from docker-java and spew it to the logger
	...
	private static class ContainerLogReader extends ResultCallback.Adapter<Frame>{
		...
		public void onNext(Frame object) {
			if(StreamType.STDOUT == object.getStreamType() ||
			   StreamType.STDERR == object.getStreamType()){
